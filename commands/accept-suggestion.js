...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDescription('Accept a suggestion by its message ID.')
    ...
    async execute(interaction) {
        ...
        try {
            ...
            await Suggestion.updateOne({ messageId }, { status: 'accepted' });

            await interaction.reply({
                content: `Suggestion **${suggestion.suggestion}** accepted.`,
                ephemeral: true
            });
        } catch (error) {
            console.error('Error accepting suggestion:', error);
            await interaction.reply({
                content: 'Failed to accept the suggestion.',
                ephemeral: true
            });
        }
    }
